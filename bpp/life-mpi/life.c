...
int main(int argc, char **argv) {
  ...
  history = malloc(MAX_AGES * sizeof(char *));
  ...
  if (rank == 0) {
    ...
    printf("Age 0:\n");
    print_field(full_field, rows, cols, 10);
  }
  ...
  while (!game_over && age < MAX_AGES) {
    char *saved = malloc(local_rows * cols * sizeof(char));
    memcpy(saved, field + cols, local_rows * cols * sizeof(char));
    history[age] = saved;
    ...
    for (int i = 0; i < age; i++) {
      local_flags[i] = match_fields(history[age], history[i], local_rows, cols);
    }
  ...
  MPI_Gatherv(history[age], local_rows * cols, MPI_CHAR, full_field, sendcounts,
